...
void main() {
	vec4 center = Positions(pc.bufId).pos[gl_InstanceIndex];
	mat4 model = rotate(translate(mat4(1.0f), center.xyz),  center.w, vec3(1.0f, 1.0f, 1.0f));
   ...
   const float edge = 1.0;
	...
	gl_Position = pc.viewproj * model * vec4(edge * (xyz - vec3(0.5)), 1.0);
