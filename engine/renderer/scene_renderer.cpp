...
namespace Rodan {
	...
	void SceneRenderer::BeginMainPass(ICommandList& cmd,
		...
		auto currentLayout = device_->GetImageLayout(finalScene_.colorTexture.image, 0);
		...
		auto currentDepthLayout = device_->GetImageLayout(finalScene_.depthTexture.image, 0);

		cmd.Barrier({
			.image = finalScene_.depthTexture.image,
			.oldLayout = currentDepthLayout,
			.newLayout = ImageLayout::DepthAttachment,
			.aspect = ImageAspect::Depth,
			});
	...
	void SceneRenderer::TransitionOpaqueSceneToReadable(ICommandList& cmd) {
		...
		for (uint32_t mip = 0; mip < opaqueScene_.mipLevels; ++mip) {
			const ImageLayout oldLayout =
				device_->GetImageLayout(opaqueScene_.texture.image, mip);

			if (oldLayout == ImageLayout::ShaderReadOnly) {
				continue;
			}
